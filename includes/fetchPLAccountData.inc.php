    ...
    if (count($dataInput) == 2) {
        ...
        if ($dataInput['period'] > 0) {
            ...
            if ($queryUseDate) {
                ...
                $sqlquery ="SELECT a.categoryID, a.categoryLabel, a.accountID, a.accountLabel, SUM(e.grandTotal) * a.classSign AS balance FROM viewAccount a LEFT JOIN (SELECT * FROM _viewEntries _e ORDER BY _e.created DESC LIMIT " . $limit * 2 . ") e ON e.accountID = a.accountID WHERE a.categoryID IN (" . $dataInput['category'] . ") AND a.accountIsActive = 'Y' GROUP BY a.accountID HAVING balance IS NOT NULL ORDER BY a.accountID ASC";
            }
        } else {
